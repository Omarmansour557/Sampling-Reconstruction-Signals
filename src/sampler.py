...
class Sampler(qtw.QWidget):
    ...
    def reconstructSignal(self, orignal_signal, analog_time, sampling_frequency):
        ...
        if(len(sampling_values) > 0):
            self.resampled_original_signal = signal.resample(
                sampling_values, len(self.analog_time))
            self.clearCanvas()
            self.plotAllSignals(self.resampled_original_signal, sampling_time,
                                sampling_values, self.analog_time, self.orignal_signal)
    ...
    def sample(self, orignal_signal, analog_time, sampling_frequency):
        ...
        if(number_of_samples):
            ...
            sampling_values = [orignal_signal[np.searchsorted(
